...
Python** from the `hiveplotlib` repository, not hand-crafted. The fixtures are produced
using hiveplotlib's **matplotlib** backend (the default), and the JS rendering code
assumes matplotlib-style kwargs (e.g. `linewidth`, `linestyle`, `alpha`). Each fixture
has a corresponding static method in `hiveplotlib.datasets`:
