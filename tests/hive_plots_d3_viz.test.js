...
describe("normalizeNodeKwargs", () => {
    ...
    it("should prioritize facecolor over color over c", () => {
        expect(normalizeNodeKwargs({ c: "red", color: "blue", facecolor: "green" })._fill).toBe("green");
...
describe("cmapToD3Interpolator", () => {
    ...
    it("should return different interpolators for different cmaps", () => {
        expect(cmapToD3Interpolator("viridis")).not.toBe(cmapToD3Interpolator("plasma"));
    });

    it("should default to viridis for unknown cmaps", () => {
        expect(cmapToD3Interpolator("not_a_cmap")).toBe(cmapToD3Interpolator("viridis"));
    });

    it("should handle undefined/null by defaulting to viridis", () => {
        expect(cmapToD3Interpolator(undefined)).toBe(cmapToD3Interpolator("viridis"));
    });
});
...
describe("marcon", () => {
    ...
    it("should support setter/getter chaining", () => {
        const m = marcon().top(10).left(20).right(30).bottom(40).width(500).height(400);
...
describe("plotNodes", () => {
    ...
    it("should use defaults when node_viz_kwargs is absent", () => {
        ...
        expect(parseFloat(firstNode.getAttribute("fill-opacity"))).toBeCloseTo(0.8, 1);
        expect(parseFloat(firstNode.getAttribute("r"))).toBeCloseTo(scatterSizeToRadius(20), 1);
    ...
    it("should apply scalar size from node_viz_kwargs", () => {
        ...
        // All axes have s: 30
        const nodes = document.querySelectorAll("circle.node");
        expect(parseFloat(nodes[0].getAttribute("r"))).toBeCloseTo(scatterSizeToRadius(30), 2);
    });
    ...
    it("should apply scalar alpha from node_viz_kwargs", () => {
        ...
        const nodes = document.querySelectorAll("circle.node");
        expect(parseFloat(nodes[0].getAttribute("fill-opacity"))).toBeCloseTo(0.8, 1);
    });
    ...
    it("should apply per-node explicit color array from node_viz_kwargs", () => {
        ...
        const data = {
            axes: {
                A: {
                    start: [1, 0], end: [5, 0], angle: 0, long_name: "A",
    ...
    it("should apply per-node size array from node_viz_kwargs", () => {
        ...
        const nodes = document.querySelectorAll("circle.node");
        expect(parseFloat(nodes[0].getAttribute("r"))).toBeCloseTo(scatterSizeToRadius(10), 2);
        expect(parseFloat(nodes[1].getAttribute("r"))).toBeCloseTo(scatterSizeToRadius(30), 2);
        expect(parseFloat(nodes[2].getAttribute("r"))).toBeCloseTo(scatterSizeToRadius(50), 2);
    });
...
describe("plotEdges", () => {
    ...
    it("should apply per-edge linewidth array", () => {
        ...
        const edges = document.querySelectorAll("path.edge");
        expect(parseFloat(edges[0].getAttribute("stroke-width"))).toBeCloseTo(1.0, 1);
        expect(parseFloat(edges[1].getAttribute("stroke-width"))).toBeCloseTo(2.0, 1);
        expect(parseFloat(edges[2].getAttribute("stroke-width"))).toBeCloseTo(3.0, 1);
    });
        ...
        const data = {
            ...
            edges: {
                A: { B: { "0": {
                    ...
                    edge_kwargs: { array: [0.2, 0.8], cmap: "viridis", clim: [0.0, 1.0] },
                } } },
            },
        };
        const { svg, x, y } = getTestSVG();
    ...
    it("should apply stroke-dasharray for dashed linestyle", () => {
        ...
        const data = {
            axes: {
                A: { start: [1, 0], end: [5, 0], nodes: { unique_id: [0], x: [2], y: [0] } },
                B: { start: [0, 1], end: [0, 5], nodes: { unique_id: [1], x: [0], y: [2] } },
            },
            edges: {
                A: { B: { "0": {
                    ids: [[0, 1]],
...
describe("plotLabels", () => {
    ...
    it("should use text-anchor 'middle' for angle 90 (upward axis)", () => {
        const data = {
            axes: {
                Up: {
                    start: [0, 1], end: [0, 5], angle: 90, long_name: "Up",
                    nodes: { unique_id: [], x: [], y: [] },
                },
            },
            edges: {},
...
describe("visualizeHivePlot", () => {
    ...
    it("should suppress labels when showLabels is false", () => {
        const data = loadFixture("minimal_hive_plot.json");
        visualizeHivePlot(data, [-5, 5], [-5, 5], 0, 0, 0, 0, 450, 450, "body", { showLabels: false });

        expect(document.querySelectorAll("path.axis").length).toBe(Object.keys(data.axes).length);
        expect(document.querySelectorAll("text.axis-label").length).toBe(0);
    });

    it("should return an SVG selection", () => {
        const data = loadFixture("minimal_hive_plot.json");
        const result = visualizeHivePlot(data, [-5, 5], [-5, 5], 0, 0, 0, 0, 450, 450, "body");
