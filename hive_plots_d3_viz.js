...
export function marcon(
  ...
  instance.render = function () {
    svg = d3
      ...
      .attr(
        "height",
        instance.innerHeight() + instance.top() + instance.bottom(),
      )
      .append("g")
...
export function plotNodes(data, svg, x, y) {
  ...
  for (let i = 0; i < axisNames.length; i++) {
    ...
    let colorScale = null;
    if (
      Array.isArray(kwargs._fill) &&
      kwargs._fill.length > 0 &&
      typeof kwargs._fill[0] === "number"
    ) {
    ...
    svg
      ...
      .attr("fill-opacity", (d, j) => {
        return Array.isArray(kwargs.alpha)
          ? kwargs.alpha[j]
          : (kwargs.alpha ?? 0.8);
      })
      .attr("stroke", (d, j) => {
        if (!kwargs.edgecolor) return "none";
        return Array.isArray(kwargs.edgecolor)
          ? kwargs.edgecolor[j]
          : kwargs.edgecolor;
      })
      .attr("class", "node");
        ...
        for (let curveIdx = 0; curveIdx < numEdges; curveIdx++) {
          ...
          if (kwargs.linestyle) {
            const dash = linestyleToDasharray(
              Array.isArray(kwargs.linestyle)
                ? kwargs.linestyle[curveIdx]
                : kwargs.linestyle,
            );
